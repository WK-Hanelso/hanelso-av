...
| `onnxruntime` CPU | 포함 | `docker/requirements.txt` |
...
빌드 컨텍스트는 `docker/`만 사용한다.

```bash
docker build -t hanelso-swm-env:latest -f docker/Dockerfile docker/
```
...
docker run --rm \
  -v "$PWD:/workspace" \
  hanelso-swm-env:latest \
  python build_input.py configs/input_e100bt25.py
```

출력은 `work/input/E100BT-25_20260716151711_00006/` 아래에 생성된다.

### 4. 추론 실행

현재 코드 기준 실제 인자는 아래와 같다.
...
출력은 `work/inference/E100BT-25_20260716151711_00006/` 아래에 생성된다.

비교 기준으로 저장된 리포트는 다음 경로에 있다.

- `work/inference/E100BT-25_20260716151711_00006/infer_report.txt`
- `work/inference/E100BT-22_20260716031426_00014/infer_report.txt`
...
- 해당 패키지가 `docker/requirements.txt`에 있는지
