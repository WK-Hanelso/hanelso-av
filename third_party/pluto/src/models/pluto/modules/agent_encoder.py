...
class AgentEncoder(nn.Module):
    ...
    def forward(self, data):
        ...
        # ONNX-safe static-shape refactor (P1/B2a).
        # Intent: only *real* agents should be encoded; padded agents must
        # contribute ZERO downstream. history_encoder (NATSequenceEncoder) is
        # per-sample independent (Conv1d over time + LayerNorm + NeighborhoodAttention1D,
        # NO BatchNorm), so running it densely over ALL rows yields the exact same
        # per-row output as the boolean-gathered path, and multiplying by the
        # valid-agent mask zeros the padded rows -> intent preserved, no
        # size-changing index, no train/eval split needed.
        x_agent = self.history_encoder(
            agent_feature.permute(0, 2, 1).contiguous()
        )
        x_agent = x_agent * valid_agent_mask.unsqueeze(-1).to(x_agent.dtype)
