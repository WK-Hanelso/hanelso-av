...
class PlanningModel(TorchModuleWrapper):
    ...
    def forward(self, data):
        ...
        # Export assumption: reference lines are always present (R > 0), so the
        # decoder branch is always taken. Pinning this to True keeps the graph
        # static (drops the ref-free else-branch) and is a no-op for normal eval
        # where R > 0 already holds.
