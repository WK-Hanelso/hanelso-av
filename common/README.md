...
| `config.py` | **계층 config 조합 로더**. ROOT config(`configs/*.py`)의 모듈 이름을 `<domain>/configs/<이름>.py`로 해석·병합해 최종 config dict 하나를 반환. `_base_` 상속(dict 재귀 병합) 지원. 모든 드라이버는 root config 경로 하나만 받는다. |
...
- 실행 로그: `agent/` EXEC 문서 (운영 문서, 로컬 전용 — git 미포함)
